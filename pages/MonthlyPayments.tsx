...
const CommonPaymentTable: FC<{ onSelectAll: (checked: boolean) => void; allSelected: boolean; header: React.ReactNode; children: React.ReactNode; }> = ({ onSelectAll, allSelected, header, children }) => (
...
const FarmerWetPaymentTable: FC<{payees: FarmerWetPayee[], onSelectOne: (id: string, checked: boolean) => void, onSelectAll: (checked: boolean) => void, allSelected: boolean, onUpdate: (id: string, field: 'adjustment', value: string) => void}> = ({payees, onSelectOne, onSelectAll, allSelected, onUpdate}) => { 
    ...
    return <CommonPaymentTable onSelectAll={onSelectAll} allSelected={allSelected} header={<>
        ...
        <th className="p-3 text-right">{t('harvestedWeightKg')}</th>
...
const ServiceProviderPaymentTable: FC<{payees: ServiceProviderPayee[], onSelectOne: (id: string, checked: boolean) => void, onSelectAll: (checked: boolean) => void, allSelected: boolean, onUpdate: (id: string, field: 'adjustment', value: string) => void}> = ({payees, onSelectOne, onSelectAll, allSelected, onUpdate}) => {
    ...
    return <CommonPaymentTable onSelectAll={onSelectAll} allSelected={allSelected} header={<>
        ...
        <th className="p-3 text-right">{t('unpaidOperations')}</th>
        <th className="p-3 text-right">{t('baseAmount')}</th>
        <th className="p-3 text-right">{t('adjustment')}</th>
        <th className="p-3 text-right font-bold">{t('netPayable')}</th>
    </>}>
        {payees.map(p => <tr key={p.id} className="border-b dark:border-gray-700/50"><td className="p-3"><Checkbox checked={p.selected} onChange={e => onSelectOne(p.id, e.target.checked)}/></td><td className="p-3">{p.name}</td><td className="p-3 text-right">{p.unpaidOperations}</td><td className="p-3 text-right">{formatCurrency(p.baseAmount, settings.localization)}</td>
        <td className="p-2 text-right"><Input type="number" value={p.adjustment} onChange={e => onUpdate(p.id, 'adjustment', e.target.value)} className="text-right !py-1" /></td>
        <td className="p-3 text-right font-bold">{formatCurrency(p.netAmount, settings.localization)}</td></tr>)}
    </CommonPaymentTable>
};
...
const PaymentReview: React.FC<{ onCancel: () => void; onConfirmSuccess: () => void; config: RunConfig }> = ({ onCancel, onConfirmSuccess, config }) => {
        ...
        if (paymentType === 'farmer_wet') {
            ...
            calculatedPayees = relevantFarmers.map(farmer => {
                ...
                const payee: FarmerWetPayee = {
                    ...
                    netWeightKg: cycles.reduce((sum, c) => sum + ((c.harvestedWeight || 0) - (c.cuttingsTakenAtHarvestKg || 0)), 0),
                    wetPrice: baseAmount / cycles.reduce((sum, c) => sum + ((c.harvestedWeight || 0) - (c.cuttingsTakenAtHarvestKg || 0)), 1),
                };
                return payee;
            ...
            const relevantProviders = serviceProviders.filter(sp => 
                cuttingOperations.some(op => op.serviceProviderId === sp.id && !op.isPaid && op.date >= startDate && op.date <= endDate)
            );

            calculatedPayees = relevantProviders.map(provider => {
                const unpaidOps = cuttingOperations.filter(op => 
                    op.serviceProviderId === provider.id &&
                    !op.isPaid &&
                    op.date >= startDate &&
                    op.date <= endDate &&
                ...
                if (unpaidOps.length === 0) return null;

                const baseAmount = unpaidOps.reduce((sum, op) => sum + op.totalAmount, 0);
                const totalLines = unpaidOps.reduce((sum, op) => sum + op.moduleCuts.reduce((s, mc) => s + mc.linesCut, 0), 0);
                const averageUnitPrice = totalLines > 0 ? baseAmount / totalLines : null;

                const payee: ServiceProviderPayee = {
                    id: provider.id,
                    type: RecipientType.SERVICE_PROVIDER,
                    name: provider.name,
                    unpaidOperations: unpaidOps.length,
                    baseAmount,
                    adjustment: 0,
                    deduction: 0,
                    netAmount: baseAmount,
                    selected: true,
                    operationIds: unpaidOps.map(op => op.id),
                    totalLines,
                    averageUnitPrice
                };
                return payee;
    ...
    return (
        <Card title={t('reviewPaymentRun')}>
            ...
            <div className="mt-4 p-4 bg-gray-100 dark:bg-gray-900/50 rounded-lg flex justify-end gap-8 font-semibold">
                <span>{t('totalPayees')}: {selectedPayees.length}</span>
                <span>{t('totalBaseAmount')}: {formatCurrency(totals.base, settings.localization)}</span>
                <span>{t('totalDeductions')}: {formatCurrency(totals.deduction, settings.localization)}</span>
                <span>{t('totalPaymentAmount')}: {formatCurrency(totals.net, settings.localization)}</span>
            </div>
...
const PaymentRunSetup: React.FC<{ onPrepare: (config: RunConfig) => void }> = ({ onPrepare }) => {
    ...
    const isFarmerPayment = runConfig.paymentType.startsWith('farmer');

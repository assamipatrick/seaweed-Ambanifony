...
const ManualSection: React.FC<{ title: string; children: React.ReactNode; defaultOpen?: boolean }> = ({ title, children, defaultOpen = false }) => {
    ...
    return (
        <Card className="mb-4 overflow-hidden transition-all hover:shadow-md">
            ...
            <div className={`transition-all duration-300 ease-in-out overflow-hidden ${isOpen ? 'max-h-[3000px] opacity-100 mt-4 pt-4 border-t border-gray-200 dark:border-gray-700/50' : 'max-h-0 opacity-0'}`}>
                {children}
            </div>
        </Card>
    );
        ...
        <div className="pb-20 max-w-5xl mx-auto font-sans">
            <div className="mb-8 text-center">
                ...
                <p className="text-gray-500 dark:text-gray-400 max-w-2xl mx-auto text-sm">
                    {t('manual_introduction_text').replace(/<[^>]*>?/gm, '')}
                </p>
            </div>
            ...
            <ManualSection title={t('manual_inventory_title')}>
                 <WorkflowDiagram steps={[
                    { title: t('stocks_sur_sites'), icon: 'MapPin', desc: t('step_desc_collection') },
                    { title: t('flow_transfer'), icon: 'Truck', desc: t('step_desc_shipping_slip') },
                    { title: t('flow_warehouse'), icon: 'Building', desc: t('step_desc_reception') },
                    { title: t('flow_pressing'), icon: 'Archive', desc: t('step_desc_processing') },
                    { title: t('flow_export'), icon: 'Globe', desc: t('step_desc_shipping') },
                ]} />
                
                <div className="text-xs text-gray-600 dark:text-gray-400 mt-2 p-2 border-l-2 border-blue-500 bg-blue-50 dark:bg-blue-900/10">
                    <strong>{t('notes')}:</strong> {t('manual_inventory_note')}
                </div>
            </ManualSection>
            ...
            {/* 6. Monitoring (NEW) */}
             <ManualSection title={t('manual_monitoring_title')}>
                 <WorkflowDiagram steps={[
                    { title: t('step_log_test'), icon: 'Beaker', desc: t('step_desc_weekly') },
                    { title: t('step_analyze_pest'), icon: 'Camera', desc: t('step_desc_ai') },
                    { title: t('step_resolve_incident'), icon: 'Check', desc: t('step_desc_action') },
                ]} />
                
                 <FormGuide 
                    title={t('guide_periodic_test')}
                    fields={[
                        { name: t('date'), description: t('date'), required: true },
                        { name: t('period'), description: t('desc_select_period'), required: true },
                        { name: t('field_water_temp'), description: t('desc_optional_api') },
                        { name: t('field_salinity'), description: t('desc_manual_entry') },
                    ]}
                />
            ...
            <div className="mt-8 text-center text-xs text-gray-400">
                <p>{t('appName')} v1.1 - {t('manual_introduction')}</p>
            </div>

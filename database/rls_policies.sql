...
CREATE OR REPLACE FUNCTION auth.has_permission(permission_name TEXT) RETURNS BOOLEAN AS $$
  SELECT EXISTS (
    SELECT 1
    FROM users u
...
CREATE OR REPLACE FUNCTION auth.user_site() RETURNS UUID AS $$
  SELECT s.id
  FROM sites s
  WHERE s.manager_id = auth.user_id()
  LIMIT 1;
$$ LANGUAGE SQL STABLE SECURITY DEFINER;
...
CREATE POLICY "Admin can update app settings" ON app_settings FOR UPDATE USING (auth.has_permission('SETTINGS_GENERAL_MANAGE'));

-- ============================================
-- ROLES POLICIES
-- ============================================
CREATE POLICY "Everyone can read roles" ON roles FOR SELECT USING (true);
CREATE POLICY "Admin can manage roles" ON roles FOR ALL USING (auth.has_permission('ROLES_MANAGE'));

-- ============================================
-- USERS POLICIES
-- ============================================
CREATE POLICY "Users can read own profile" ON users FOR SELECT USING (id = auth.user_id() OR auth.has_permission('USERS_VIEW'));
CREATE POLICY "Users can update own profile" ON users FOR UPDATE USING (id = auth.user_id());
CREATE POLICY "Admin can manage users" ON users FOR ALL USING (auth.has_permission('USERS_VIEW'));

-- ============================================
-- INVITATIONS POLICIES
-- ============================================
CREATE POLICY "Admin can view invitations" ON invitations FOR SELECT USING (auth.has_permission('USERS_INVITE'));
CREATE POLICY "Admin can create invitations" ON invitations FOR INSERT WITH CHECK (auth.has_permission('USERS_INVITE'));
CREATE POLICY "Admin can update invitations" ON invitations FOR UPDATE USING (auth.has_permission('USERS_INVITE'));

-- ============================================
-- CONFIGURATION TABLES POLICIES
-- ============================================
CREATE POLICY "Everyone can read incident types" ON incident_types FOR SELECT USING (true);
CREATE POLICY "Admin can manage incident types" ON incident_types FOR ALL USING (auth.has_permission('SETTINGS_GENERAL_MANAGE'));

CREATE POLICY "Everyone can read incident severities" ON incident_severities FOR SELECT USING (true);
CREATE POLICY "Admin can manage incident severities" ON incident_severities FOR ALL USING (auth.has_permission('SETTINGS_GENERAL_MANAGE'));

CREATE POLICY "Everyone can read seaweed types" ON seaweed_types FOR SELECT USING (true);
CREATE POLICY "Admin can manage seaweed types" ON seaweed_types FOR ALL USING (auth.has_permission('SETTINGS_GENERAL_MANAGE'));

CREATE POLICY "Everyone can read seaweed price history" ON seaweed_price_history FOR SELECT USING (true);
CREATE POLICY "Admin can manage seaweed price history" ON seaweed_price_history FOR ALL USING (auth.has_permission('SETTINGS_GENERAL_MANAGE'));

CREATE POLICY "Everyone can read credit types" ON credit_types FOR SELECT USING (true);
CREATE POLICY "Admin can manage credit types" ON credit_types FOR ALL USING (auth.has_permission('SETTINGS_GENERAL_MANAGE'));

-- ============================================
-- SITES AND ZONES POLICIES
-- ============================================
CREATE POLICY "Everyone can read sites" ON sites FOR SELECT USING (auth.has_permission('SITES_VIEW'));
CREATE POLICY "Admin can manage sites" ON sites FOR ALL USING (auth.has_permission('SITES_MANAGE'));

CREATE POLICY "Everyone can read zones" ON zones FOR SELECT USING (auth.has_permission('SITES_VIEW'));
CREATE POLICY "Admin can manage zones" ON zones FOR ALL USING (auth.has_permission('SITES_MANAGE'));

-- ============================================
-- STAKEHOLDERS POLICIES
-- ============================================
CREATE POLICY "Authorized users can read employees" ON employees FOR SELECT USING (auth.has_permission('STAKEHOLDERS_VIEW'));
CREATE POLICY "Admin can manage employees" ON employees FOR ALL USING (auth.has_permission('EMPLOYEES_MANAGE'));

CREATE POLICY "Authorized users can read farmers" ON farmers FOR SELECT USING (auth.has_permission('STAKEHOLDERS_VIEW'));
CREATE POLICY "Admin can manage farmers" ON farmers FOR ALL USING (auth.has_permission('FARMERS_MANAGE'));

CREATE POLICY "Authorized users can read service providers" ON service_providers FOR SELECT USING (auth.has_permission('STAKEHOLDERS_VIEW'));
CREATE POLICY "Admin can manage service providers" ON service_providers FOR ALL USING (auth.has_permission('EMPLOYEES_MANAGE'));

-- ============================================
-- MODULES POLICIES
-- ============================================
CREATE POLICY "Authorized users can read modules" ON modules FOR SELECT USING (auth.has_permission('MODULES_VIEW'));
CREATE POLICY "Admin can manage modules" ON modules FOR ALL USING (auth.has_permission('MODULES_MANAGE'));

-- ============================================
-- OPERATIONS POLICIES
-- ============================================
CREATE POLICY "Authorized users can read cutting operations" ON cutting_operations 
  FOR SELECT USING (auth.has_permission('OPERATIONS_VIEW'));
CREATE POLICY "Operations staff can manage cutting operations" ON cutting_operations 
  FOR ALL USING (auth.has_permission('OPERATIONS_VIEW'));

CREATE POLICY "Authorized users can read cultivation cycles" ON cultivation_cycles 
  FOR SELECT USING (auth.has_permission('CYCLES_VIEW'));
CREATE POLICY "Operations staff can manage cycles" ON cultivation_cycles 
  FOR ALL USING (auth.has_permission('CYCLES_MANAGE'));

-- ============================================
-- INVENTORY POLICIES
-- ============================================
CREATE POLICY "Authorized users can read stock movements" ON stock_movements 
  FOR SELECT USING (auth.has_permission('INVENTORY_VIEW'));
CREATE POLICY "Inventory managers can create stock movements" ON stock_movements 
  FOR INSERT WITH CHECK (auth.has_permission('INVENTORY_MANAGE_ON_SITE'));
...
CREATE POLICY "Inventory managers can manage farmer deliveries" ON farmer_deliveries 
  FOR ALL USING (auth.has_permission('INVENTORY_MANAGE_ON_SITE'));
...
CREATE POLICY "Export managers can manage export documents" ON export_documents 
  FOR ALL USING (auth.has_permission('EXPORTS_MANAGE'));
...
CREATE POLICY "Everyone can create periodic tests" ON periodic_tests 
  FOR INSERT WITH CHECK (auth.has_permission('MONITORING_VIEW'));
CREATE POLICY "Conductor can update own tests" ON periodic_tests 
  FOR UPDATE USING (conductor_id = auth.user_id() OR auth.has_permission('INCIDENTS_MANAGE'));
...
CREATE POLICY "Authorized users can read message logs" ON message_logs 
  FOR SELECT USING (auth.has_permission('DASHBOARD_VIEW'));
...
CREATE POLICY "Authorized users can read gallery photos" ON gallery_photos 
  FOR SELECT USING (auth.has_permission('GALLERY_VIEW'));
CREATE POLICY "Gallery managers can manage photos" ON gallery_photos 
  FOR ALL USING (auth.has_permission('GALLERY_MANAGE'));
